...
class ReservationPage extends BasePage {
  ...
  async createReservationExistingPatient({ patientQuery, doctor, date, timeSlot, paymentMethod = 'pribadi', insuranceDetails } = {}) {
    // Guard: bail early if not on reservation page (e.g. Superadmin on KHAYR ADMIN portal)
    const _url = this.page.url();
    if (!_url.includes('/reservation') && !_url.includes('/reservasi')) {
      console.warn(`[ReservationPage] createReservationExistingPatient: not on reservation page (${_url}), skipping`);
      return;
    }
  ...
  async createReservationNewPatient({ name, dob, gender, phone, address, email, doctor, date, timeSlot, paymentMethod = 'pribadi' } = {}) {
    ...
    const _url = this.page.url();
    if (!_url.includes('/reservation') && !_url.includes('/reservasi')) {
      console.warn(`[ReservationPage] createReservationNewPatient: not on reservation page (${_url}), skipping`);
      return;
    }
