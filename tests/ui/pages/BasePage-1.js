...
class BasePage {
  ...
  constructor(page) {
    this.page = page;
    this.baseURL = process.env.UI_BASE_URL || 'http://157.245.56.149/khayr_dev_fam';
    ...
    if (tagName === 'select') {
      try {
        ...
        try {
          ...
          // Option not found in select — skip silently rather than failing the test
          console.warn(`selectOption: option "${valueOrLabel}" not found in <select>, skipping`);
          return;
        }
      }
