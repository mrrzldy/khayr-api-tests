...
class ReportsPage extends BasePage {
  ...
  constructor(page) {
    ...
    // Direct URLs
    this.urlFinancial = '/report/finance';
    this.urlPerformance = '/report/performance';
  ...
  async assertFinancialTabContentVisible(tabType = '') {
    const container = this.financialReportContainer.or(this.page.locator('.card, table, [role="main"], .report-wrapper, body').first());
    await expect(container).toBeVisible({ timeout: this.defaultTimeout });
  }

  async assertPerformanceTabContentVisible(tabName = '') {
    const container = this.performanceReportContainer.or(this.page.locator('.card, table, [role="main"], .kpi-wrapper, canvas, svg, body').first());
    await expect(container).toBeVisible({ timeout: this.defaultTimeout });
  }
  ...
  async assertValidationError(expectedMessage = null) {
    await expect(this.validationError.or(this.page.locator('.invalid-feedback, .alert, input:invalid').first())).toBeVisible({ timeout: this.defaultTimeout });
    if (expectedMessage) {
